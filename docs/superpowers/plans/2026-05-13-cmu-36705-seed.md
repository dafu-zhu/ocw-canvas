# CMU 36-705 Seed Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a second seed course — `CMU 36-705 (Intermediate Statistics)` — as `backend/seed/seed_cmu_36705.py`, mirroring `seed_template_course.py`'s shape but adapted for Larry Wasserman's CMU page (no videos; 27 lecture-note PDFs with two `a`-variant slots; 13 HW PDFs with mixed-case filenames; 2 in-term tests + 1 final, none with source papers).

**Architecture:** Single new file under `backend/seed/`. Module-level constants for URLs / coverage / titles; one `seed(db, force)` builder; one `update_urls(db)` in-place refresher; one argparse `main()`. No model or migration changes — strictly additive.

**Tech Stack:** Python 3.12 · SQLAlchemy 2 ORM · existing `app.db.SessionLocal` · `argparse`. Verification uses a local SQLite DB created via `alembic upgrade head`.

**Spec:** `docs/superpowers/specs/2026-05-13-cmu-36705-seed-design.md`.

**Convention note:** Existing seed (`seed_template_course.py`) has zero pytest coverage and is verified by manual smoke-run. We follow the same convention — no new pytest cases; verification is `seed` + `update_urls` round-trip on a fresh SQLite DB plus a HEAD-fetch spot-check of representative URLs.

---

### Task 1: Branch + file skeleton (URL helpers and constants only)

**Files:**
- Create: `backend/seed/seed_cmu_36705.py`

- [ ] **Step 1: Create a feature branch**

```bash
git checkout -b feat/seed-cmu-36705
```

- [ ] **Step 2: Create the seed file with module docstring, imports, base URLs, and URL helpers**

Write `backend/seed/seed_cmu_36705.py` containing exactly:
...
- [ ] **Step 3: Verify the helpers in a Python REPL**

Run (from `backend/`):
```
uv run python -c "from seed.seed_cmu_36705 import LECTURE_NOTE_URLS, HOMEWORK_URLS; print(LECTURE_NOTE_URLS[11]); print(LECTURE_NOTE_URLS[12]); print(LECTURE_NOTE_URLS[13]); print(HOMEWORK_URLS[1]); print(HOMEWORK_URLS[2]); print(len(LECTURE_NOTE_URLS), len(HOMEWORK_URLS))"
```

Expected output:
```
https://www.stat.cmu.edu/~larry/=stat705/Lecture11a.pdf
https://www.stat.cmu.edu/~larry/=stat705/Lecture12a.pdf
https://www.stat.cmu.edu/~larry/=stat705/Lecture13.pdf
https://www.stat.cmu.edu/~larry/=stat705/homework1.pdf
https://www.stat.cmu.edu/~larry/=stat705/Homework2.pdf
27 13
```

---

### Task 2: Static tables and markdown blobs

**Files:**
- Modify: `backend/seed/seed_cmu_36705.py` (append)

- [ ] **Step 1: Append the HW coverage table, test/final coverage constants, lecture-title table, and unit grouping**

Append to `backend/seed/seed_cmu_36705.py` (after the URL constants from Task 1):
...
- [ ] **Step 2: Append the markdown blobs (home, syllabus, description, textbook)**

Append to `backend/seed/seed_cmu_36705.py`:
...
## Textbooks
- **Primary** — Wasserman, *All of Statistics: A Concise Course in Statistical Inference* (2004). Covers Chapters 1-12 + supplementary material.
...
- [ ] **Step 3: Append the assignment-description builders**
...
- [ ] **Step 4: Verify the table + builders import cleanly**

Run (from `backend/`):
```
uv run python -c "from seed.seed_cmu_36705 import HOMEWORKS, UNITS, LECTURE_TOPICS, _hw_description, _test_description, _final_description; assert sum((hi - lo + 1) for _, lo, hi, _ in UNITS) == 27, 'unit coverage != 27'; assert sum((hi - lo + 1) for _, _, lo, hi in HOMEWORKS) == 27, 'hw coverage != 27'; assert len(LECTURE_TOPICS) == 27; print('OK')"
```
...
### Task 3: Module/unit tree builder

**Files:**
- Modify: `backend/seed/seed_cmu_36705.py` (append)

- [ ] **Step 1: Append the `_units()` builder**
...
def _modules() -> list[tuple[str, list[dict]]]:
    ...
    test_stops = {TEST1_COVERS_TO: ("Test I", "Test I"),
                  TEST2_COVERS_TO: ("Test II", "Test II")}
    for title, lo, hi, readings_md in UNITS:
        out.append((title, _unit_items(lo, hi, readings_md)))
        if hi in test_stops:
            label, _ = test_stops[hi]
            out.append(
                (label, [{"kind": "assignment", "_assignment": label}])
            )
    out.append(
        ("Final Exam", [{"kind": "assignment", "_assignment": "Final Exam"}])
    )
    return out
```
...
Run (from `backend/`):
```
uv run python -c "from seed.seed_cmu_36705 import _modules; ms = _modules(); print(len(ms), 'modules'); [print(t, '->', len(items), 'items') for t, items in ms]"
```

Expected output (11 modules; counts shown):
```
11 modules
Direct links -> 4 items
Unit 1 — Foundations: Probability Review, Concentration, Convergence & CLT -> 6 items
Unit 2 — Empirical Process Theory -> 3 items
Test I -> 1 items
Unit 3 — Point Estimation & Decision Theory -> 6 items
Unit 4 — Hypothesis Testing & Multiple Testing -> 4 items
Unit 5 — Confidence Intervals -> 4 items
Test II -> 1 items
Unit 6 — Bootstrap & Bayesian Inference -> 5 items
Unit 7 — Modern Statistics: Regression, Minimax, High-Dim, Model Selection -> 6 items
Final Exam -> 1 items
```

(Item counts: each unit = (hi - lo + 1) lecture links + 1 Readings note. U1=5+1=6, U2=2+1=3, U3=5+1=6, U4=3+1=4, U5=3+1=4, U6=4+1=5, U7=5+1=6.)

---

### Task 4: `seed()` function

**Files:**
- Modify: `backend/seed/seed_cmu_36705.py` (append)

- [ ] **Step 1: Append `_delete_existing` and `seed`**
...
Run (from `backend/`):
```
uv run python -c "from seed.seed_cmu_36705 import seed; print(seed.__doc__ or 'imported ok')"
```
...
### Task 5: `update_urls()` in-place refresher

**Files:**
- Modify: `backend/seed/seed_cmu_36705.py` (append)

- [ ] **Step 1: Append the regex constants and `update_urls()` function**
...
- [ ] **Step 2: Verify `update_urls` imports**

Run (from `backend/`):
```
uv run python -c "from seed.seed_cmu_36705 import update_urls; print('ok')"
```
...
### Task 6: `main()` argparse entry point

**Files:**
- Modify: `backend/seed/seed_cmu_36705.py` (append)

- [ ] **Step 1: Append the `main()` entrypoint**
...
- [ ] **Step 2: Verify `--help` shows the flags**

Run (from `backend/`):
```
uv run python -m seed.seed_cmu_36705 --help
```

Expected output (key lines):
```
usage: seed_cmu_36705.py [-h] [--force] [--update-urls]
...
  --force         delete an existing CMU 36-705 course first
  --update-urls   only refresh module_item external_url + assignment fields on the existing course (no deletions)
```

---

### Task 7: Smoke-test against a fresh SQLite DB

**Files:** _no source changes_

- [ ] **Step 1: Build a fresh SQLite DB**

Run (from `backend/`):
```
rm -f _seed_smoke.db
DATABASE_URL=sqlite:///./_seed_smoke.db uv run alembic upgrade head
```

PowerShell variant:
```
Remove-Item _seed_smoke.db -ErrorAction SilentlyContinue
$env:DATABASE_URL='sqlite:///./_seed_smoke.db'
uv run alembic upgrade head
```

Expected: a sequence of `INFO  [alembic.runtime.migration] Running upgrade ...` lines ending at `0005`, and the file `_seed_smoke.db` is created.

- [ ] **Step 2: Seed the course**

Run (from `backend/`):
```
DATABASE_URL=sqlite:///./_seed_smoke.db uv run python -m seed.seed_cmu_36705
```

PowerShell:
```
uv run python -m seed.seed_cmu_36705
```

Expected output:
```
Seeded CMU 36-705 — Intermediate Statistics: 11 modules, 41 items, 16 assignments.
```

(Items: 4 direct-links + U1=6 + U2=3 + T1=1 + U3=6 + U4=4 + U5=4 + T2=1 + U6=5 + U7=6 + Final=1 = 41 items. Assignments: 13 HW + 2 tests + 1 final = 16.)

- [ ] **Step 3: Verify idempotency — running again is a no-op**
...
Expected output: same summary line (no duplicate insert, no exception).

- [ ] **Step 4: Verify `--update-urls` is idempotent (everything = 0 the second run)**

Run twice:
```
DATABASE_URL=sqlite:///./_seed_smoke.db uv run python -m seed.seed_cmu_36705 --update-urls
DATABASE_URL=sqlite:///./_seed_smoke.db uv run python -m seed.seed_cmu_36705 --update-urls
```

PowerShell:
```
uv run python -m seed.seed_cmu_36705 --update-urls
uv run python -m seed.seed_cmu_36705 --update-urls
```

Expected (the second run):
```
update_urls: course_found=True, items_examined=<N>, items_updated=0, assignments_updated=0, coverage_updated=0
```

- [ ] **Step 5: Spot-check generated URLs in the DB**

Run (from `backend/`):
```
DATABASE_URL=sqlite:///./_seed_smoke.db uv run python -c "
from app.db import SessionLocal
from app.models import Course, ModuleItem
db = SessionLocal()
c = db.query(Course).filter(Course.code == 'CMU 36-705').first()
urls = sorted({it.external_url for m in c.modules for it in m.items if it.external_url})
for u in urls: print(u)
"
```

Expected: 28-ish distinct URLs (1 home, 1 syllabus, 1 Piazza, 25 unique lecture-note PDFs since LN11=11a and LN12=12a are distinct slugs). Verify two specific lines appear in the output:
```
https://www.stat.cmu.edu/~larry/=stat705/Lecture11a.pdf
https://www.stat.cmu.edu/~larry/=stat705/Lecture12a.pdf
```

And that **no** plain `Lecture11.pdf` or `Lecture12.pdf` line appears.

- [ ] **Step 6: Clean up the smoke DB**
...
PowerShell:
```
Remove-Item _seed_smoke.db
```

---

### Task 8: Lint, type-check, and pytest
...
Expected: `All checks passed!` (or no errors on the new file). If ruff flags `E501` line-too-long on any of the markdown blob strings, hard-wrap the offending line at ≤100 chars.
...
Expected: same green test count as before this change (we added no tests, and we didn't touch app code). If anything red, the cause is unrelated to this seed — investigate, but do not modify app code without a follow-up plan.

- [ ] **Step 3: Frontend untouched, but sanity-check CI doesn't trip**
...
Expected: `Found 0 errors`. (We didn't touch frontend at all; this just confirms the CI matrix won't trip on something else.)
...
Run:
```
git add backend/seed/seed_cmu_36705.py docs/superpowers/specs/2026-05-13-cmu-36705-seed-design.md docs/superpowers/plans/2026-05-13-cmu-36705-seed.md
```

- [ ] **Step 2: Verify staged diff is just these three files**
...
Expected: `Changes to be committed:` lists exactly those three files; `Changes not staged for commit:` is empty (or unrelated files only).

- [ ] **Step 3: Commit**

Run:
```
git commit -m "feat(seed): add CMU 36-705 (Intermediate Statistics) seed course

Larry Wasserman's CMU page. 11 modules (7 topic units + 2 in-term tests +
final + direct-links), 16 assignments (13 HWs at 50% / Test I 10% /
Test II 10% / Final 30%, drop_lowest_n=0 throughout, faithful to syllabus).

Pattern divergence vs the 18.100B seed: no video items (source has no
public recordings), lecture-note slots 11/12 use 11a/12a, HW1 lowercase
vs HW2-13 Title-case, tests/final have no source paper. term_label='' by
default (user picks self-study term in teacher mode).

Spec: docs/superpowers/specs/2026-05-13-cmu-36705-seed-design.md
Plan: docs/superpowers/plans/2026-05-13-cmu-36705-seed.md"
```

- [ ] **Step 4: Decide on PR**

The repo's CLAUDE.md says branches merge to master via `--no-ff` after CI passes. Since the user requested unattended implementation, push the branch and open a PR for review rather than self-merging:

```
git push -u origin feat/seed-cmu-36705
gh pr create --title "feat(seed): add CMU 36-705 (Intermediate Statistics) seed course" --body "$(cat <<'EOF'
## Summary
- Adds backend/seed/seed_cmu_36705.py — second seed course after MIT 18.100B.
- 11 modules (Direct links + 7 topic units + Test I + Test II + Final Exam).
- 16 assignments (13 HWs · Test I · Test II · Final), no drops.
- Adapts to CMU's source quirks: no videos, Lecture11a/12a slots, mixed-case HW filenames, no test/final paper.
- term_label='' by design — user picks self-study term in teacher mode.

## Test plan
- [x] uv run ruff check . — clean
- [x] uv run pytest -q — green (no test count change)
- [x] DATABASE_URL=sqlite:///./_seed_smoke.db uv run alembic upgrade head — runs to 0005
- [x] uv run python -m seed.seed_cmu_36705 — "Seeded CMU 36-705 ...: 11 modules, ... items, 16 assignments."
- [x] Re-run is a no-op (idempotency)
- [x] uv run python -m seed.seed_cmu_36705 --update-urls — second run reports all-zeros
- [x] No `Lecture11.pdf` / `Lecture12.pdf` URLs in the DB; only `Lecture11a.pdf` / `Lecture12a.pdf`
EOF
)"
```

If `gh` is not authenticated, skip the PR creation and just `git push -u origin feat/seed-cmu-36705`; report the branch name back to the user.

---

## Self-review notes

- **Spec coverage:** Every section of `docs/superpowers/specs/2026-05-13-cmu-36705-seed-design.md` maps to a Task above. §2 (source structure) → Task 1; §3 (course meta) + §4 (groups) + §5 (assignments) → Task 4; §6 (modules) → Tasks 2-3; §7 (URL helpers) → Task 1; §8 (file layout) → Task 6; §10 (testing) → Task 7. §9 / §11 / §12 are commentary and need no implementation tasks.
- **No placeholders:** Every step contains the actual code or command. No "TBD", no "similar to Task N" without repeating the code.
- **Type consistency:** `LECTURE_TOPICS` / `LECTURE_NOTE_URLS` / `HOMEWORK_URLS` are dicts keyed by `int`. `HOMEWORKS` is `list[tuple[int, str, int, int]]`. `UNITS` is `list[tuple[str, int, int, str]]`. `_modules()` returns `list[tuple[str, list[dict]]]`. These shapes are consistent across tasks.
- **Item-count caveat:** Task 7 Step 2's "40 items" is computed as `(11 modules' items: 4 direct + 6 + 3 + 1 + 6 + 4 + 4 + 1 + 5 + 6 + 1) = 41`. The expected number is **41**, not 40 — fixed in the step. If the actual output differs, the failure mode is a counted-vs-rendered discrepancy in `_modules()`; investigate `_unit_items()`.
